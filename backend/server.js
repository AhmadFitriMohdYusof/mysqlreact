...
app.get("/", (req, res) => {
    const sql = "SELECT * FROM student4"; // SQL query to fetch all students
...
app.post("/create", (req, res) => {
    const sql = "INSERT INTO student4(name,email,marks,grade,city) VALUES (?)"; // SQL query to insert a new student
    const values = [
...
app.put("/update/:id", (req, res) => {
    const sql = "UPDATE student4 SET `name` = ?, `email` = ?, `marks` = ?, `grade` = ?, `city` = ? WHERE id = ?"; // SQL query to update student details
    const values = [
        req.body.name, // Get name from request body
        req.body.email, // Get email from request body
...
app.delete("/student4/:id", (req, res) => {
    const sql = "DELETE FROM student4 WHERE id = ?"; // SQL query to delete a student
    const id = req.params.id // Get student ID from route parameters
